...
class ZhiPu(Protocol):
    ...
    def session_update(self):
        ...
        if self.turn_detection_type == TurnDetectionType.SERVER_VAD:
            ...
            config["output_audio_format"] = "pcm"
        self.__send_event({
            "type": "session.update",
            "session": config,
        })
    ...
    def send_audio(self, audio_bytes: bytes):
        ...
        self.__send_event({"type": "input_audio_buffer.commit", "client_timestamp": int(time.time() * 1000)})
    ...
    def stream_audio(self, audio_chunk: bytes):
        ...
        self.__send_event({"type": "input_audio_buffer.append", "audio": audio_b64, "client_timestamp": int(time.time() * 1000)})
    ...
    def clear_audio(self):
        ...
        if self._is_start == False:
            raise('请启动后再调用')
        self.__send_event({"type": "input_audio_buffer.clear", "client_timestamp": int(time.time() * 1000)})
    ...
    def send_audio_image(self, audio_bytes: bytes, image_bytes: bytes):
        ...
        audio_b64 = base64.b64encode(audio_bytes).decode()
        self.__send_event({"type": "input_audio_buffer.append", "audio": audio_b64, "client_timestamp": int(time.time() * 1000)})
        self.__send_event({"type": "input_audio_buffer.append_video_frame", "video_frame": image_b64, "client_timestamp": int(time.time() * 1000)})
        self.__send_event({"type": "input_audio_buffer.append_video_frame", "video_frame": image_b64, "client_timestamp": int(time.time() * 1000)})
    ...
    def cancel_response(self):
        ...
        self.__send_event({"type": "response.cancel", "client_timestamp": int(time.time() * 1000)})
