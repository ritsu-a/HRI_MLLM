...
class SourceAwareMixedDatasetVQ(Text2MotionDataset):
    def __init__(
        self,
        ...
        **kwargs,
            ...
            for name in temp_dataset.name_list:
                ...
                if motion.shape[0] >= self.window_size:
                    ...
                    self.all_data_dict[prefixed_name] = {
                        ...
                        "source": dataset_idx,  # 记录数据源
                        "source_name": ["BEAT_v2_kimi", "SeG_kimi"][dataset_idx]  # 数据源名称
                    }
                    self.all_samples.append({
                        ...
                        "source": dataset_idx,
                        "source_name": ["BEAT_v2_kimi", "SeG_kimi"][dataset_idx]
                    })
        ...
        print(f"Source-aware mixed dataset loaded:")
        for i, (data_root, weight) in enumerate(zip(data_root_list, self.dataset_weights)):
            ...
            source_name = ["BEAT_v2_kimi", "SeG_kimi"][i]
            print(f"  {source_name}: {dataset_count} samples, weight: {weight}")
    ...
    def __getitem__(self, item):
        ...
        motion, length, source, source_name = data["motion"], data["length"], data["source"], data["source_name"]
        ...
        # 返回数据、标签、长度、源信息等
        # 格式: (text, motion, length, source, source_name, extra1, extra2)
        return None, motion, length, source, source_name, None, None
