...
class Fine_GPT2_2part(nn.Module):
    ...
    def forward(self, idxs, cond, targets=None):
        ...
        x = self.drop(token_embeddings + position_embeddings)       # token embeddings [68, 45, 768]     pe [1, 45, 768]

        x = self.blocks(x)                      # [68, 45, 768]
        # x = self.ln_f(x)

        x = self.ln_f(x)                    # [68, 45, 768]
        N, T, C = x.size()
        t = T // self.n_modality
        # 在这里舍弃掉了[:, 0:t]的与audio信息相关的部分
        logits_body = self.head_body(x[:, t:t * 2, :])           # [68, 15, 512]
