...
from HRI_mllm.utils.motion_utils.beatml3d import feats2joints
from HRI_mllm.utils.motion_utils.metrics import calc_mpjpe, calc_pampjpe
from HRI_mllm.external.HRI_retarget.HRI_retarget.utils.io.beat_humanml3d_representation import vec_to_data_pkl
import torch.nn.functional as F
import numpy as np
from tqdm import tqdm

SAVE_DIR = os.path.join(Path(ROOT).parent,"output", "beat_motion")
...
# 初始化 wandb
wandb.init(mode='online', project="beat-vqvae")  # 替换为你的 wandb 用户名

# 加载数据集
# 对啊，这个collate_fn肯定得自己重写，不能用它humanml3d的。之前代码看得我一愣一愣的
...
def compute_loss(features, x_out, quant_loss, beta: float = 0.25):
    ...
    # 这里是回归问题 我觉得用L2 loss可能比原论文里的likelihood要好一点？
...
def train_vqvae(config, train_loader, val_loader):
    ...
    for epoch in tqdm(range(config["epochs"])):
        ...
        # 保存模型 checkpoint
        if epoch % 5 == 0:
            torch.save(motion_vae.state_dict(), f"{SAVE_DIR}/vqvae_epoch_{epoch}.pt")
            wandb.save(f"{SAVE_DIR}/beat_vqvae_epoch_{epoch}.pt")  # 上传到 wandb
...
if __name__ == "__main__":
    ...
    # 保存最终模型
    torch.save(trained_vae.state_dict(), f"{SAVE_DIR}/vqvae_final.pt")
    wandb.save(f"{SAVE_DIR}/vqvae_final.pt")  # 上传到 wandb